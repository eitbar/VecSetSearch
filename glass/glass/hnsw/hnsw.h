...
struct HNSWSQ8 {
  int nb, dim;
  int M, efConstruction;
  std::unique_ptr<hnswlib::HierarchicalNSW<int32_t>> hnsw = nullptr;
  std::unique_ptr<hnswlib::SpaceInterface<int32_t>> space = nullptr;

  Graph<int> final_graph;

  HNSWSQ8(int dim, const std::string &metric, int R = 32, int L = 200)
      : dim(dim), M(R), efConstruction(L) {
    auto m = metric_map[metric];
    if (m == Metric::L2SQ8) {
      space = std::make_unique<hnswlib::L2SpaceSQ8>(dim);
    } else {
      printf("Unsupported metric type\n");
    }
  }

  void Build(uint8_t *data, int N) {
    nb = N;
    hnsw = std::make_unique<hnswlib::HierarchicalNSW<int32_t>>(space.get(), N, M,
                                                             efConstruction);
    std::atomic<int> cnt{0};
    auto st = std::chrono::high_resolution_clock::now();
    hnsw->addPoint(data, 0);
